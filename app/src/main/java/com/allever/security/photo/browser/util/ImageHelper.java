...
//                        if (MediaFile.isGifFileType(result.mPath)) {
//                            result.mType = MediaTypeUtil.INSTANCE.getTYPE_GIF();
//                        } else if (MediaFile.isJPGFileType(result.mPath)) {
//                            result.mType = MediaTypeUtil.INSTANCE.getTYPE_JPG();
//                        } else if (MediaFile.isPNGFileType(result.mPath)) {
//                            result.mType = MediaTypeUtil.INSTANCE.getTYPE_PNG();
//                        } else {
//                            result.mType = MediaTypeUtil.INSTANCE.getTYPE_OTHER_IMAGE();
//                        }
//                    }
//                }
//            } else {
...
//		try {
//			if(PhoneInfo.isSupportWriteExtSdCard() && ExtSdcardUtils.INSTANCE.isExtSdcardPath(path)){//如果是外置SDCARD
//				String pathName = path + File.separator + filename;
//				OutputStream outputStream = ExtSdcardUtils.INSTANCE.getExtCardOutputStream(context, pathName, "");//不传入mimeType防止它直接加入Media数据库
...
//        try {
//            if (PhoneInfo.isSupportWriteExtSdCard() && ExtSdcardUtils.INSTANCE.isExtSdcardPath(path)) {//如果是外置SDCARD  由于EXIF文件只能传入路径 为了防止EXIF文件写入失败 先写入图片至内置SDCARD然后再保存到外置
//
//                //将图片转成byte数组
...
//                    bb.setDegree(cursor.getInt(degreeIndex));
                    if (MediaFile.isGifFileType(bb.getPath())) {
                        ...
                        bb.setType(MediaTypeUtil.INSTANCE.getTYPE_PNG());
                    } else {
                        bb.setType(MediaTypeUtil.INSTANCE.getTYPE_OTHER_IMAGE());
                    }
                    result.add(bb);
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                do {
                    ...
                    bb.setDate(cursor.getLong(dateIndex));
                    bb.setType(MediaTypeUtil.INSTANCE.getTYPE_VIDEO());
...
//                    bb.setDegree(c1.getInt(degreeIndex));
    ...
    public static int deleteMediaByBucketId(Context context, ImageFolder folder, IDeleteListener listener) {
        ...
        if (ExtSdcardUtils.INSTANCE.isExtSdcardPath(folder.getDir())) {//这种情况删除时需要特殊处理  要先删除文件 如果是外置路径 且支持外置SDCARD
...
//				String path = FileUtil.getParentFilePath(thumbnailBeanArrayList.get(0).getPath());
                return ExtSdcardUtils.INSTANCE.deleteDirectoryFile(context, thumbnailBeanArrayList, listener);
...
//                if (ExtSdcardUtils.INSTANCE.isExtSdcardPath(path)) {//如果是外置路径
//                    if (PhoneInfo.isSupportWriteExtSdCard()) {//支持外置SDCARD 这种情况删除时需要特殊处理  要先删除文件
//                        int success = ExtSdcardUtils.INSTANCE.deleteDirectoryFile(context, thumbnailBeens, listener);
...
//                }
//
//                if (ExtSdcardUtils.INSTANCE.isExtSdcardPath(path)) {//如果是外置路径 且支持外置SDCARD
//                    if (PhoneInfo.isSupportWriteExtSdCard()) {//这种情况删除时需要特殊处理  要先删除文件
//                        boolean success = ExtSdcardUtils.INSTANCE.deleteExtFile(context, path);
